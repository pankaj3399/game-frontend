...
import { IconMap } from "@/icons/figma-icons";
    ...
    <div className="rounded-[12px] border border-border bg-card p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1 overflow-hidden">
          <p
            className="text-pretty text-sm font-semibold leading-snug text-foreground [overflow-wrap:anywhere]"
            title={headline}
            tabIndex={0}
          >
            {headline}
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-muted-foreground">
            <span className="shrink-0">{match.scheduledText}</span>
            <span aria-hidden className="text-border">
              ·
            </span>
            <span className="inline-flex min-w-0 items-center gap-1">
              <IconMap size={14} className="shrink-0 text-muted-foreground" />
              <span className="min-w-0 truncate" title={match.courtLabel}>
                {match.courtLabel}
              </span>
            </span>
          </p>
