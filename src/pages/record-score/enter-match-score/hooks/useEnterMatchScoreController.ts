...
import {
  ...
  useScoreQrRequesterSessionEvents,
} from "@/pages/tournaments/hooks/useTournamentScoreQr";
import { useQrScoreDraftSync } from "./useQrScoreDraftSync";
import {
...
export function useEnterMatchScoreController({
  ...
  const opponentConsumedHandledRef = useRef(false);
  ...
  const generateRows =
    scoreDraftRows?.matchKey === scoreDraftMatchKey
      ? scoreDraftRows.rows
      : shouldUseHydratedState && hydratedRows
        ? hydratedRows
        : rows;
  ...
  const buildCurrentScoreInput = useCallback(
    (showErrorToast = true): RecordTournamentMatchScoreInput | null => {
  ...
  const qrScoreSyncEnabled =
    mode === "generate" &&
    Boolean(activeScoreQrRequestId) &&
    (hasPendingQrSession || Boolean(generatedValidationUrl));

  const qrScoreSync = useQrScoreDraftSync({
    enabled: qrScoreSyncEnabled,
    requestId: activeScoreQrRequestId,
    draftKey: currentQrRequestKey,
    buildInput: buildCurrentScoreInput,
    onSyncError: (message) => {
      toast.error(
  ...
  const hasUnsavedQrChanges = qrScoreSync.isDirty;
  const isSyncingQrScores = qrScoreSync.isSyncing;
  const isScoreEntryLocked = isConfirmLocked;
  const isPrimaryGenerateDisabled =
    isGenerating ||
    isQrSessionBusy ||
    Boolean(effectiveSelectedOption.hasRecordedScore);

  const requesterQrToken =
    mode === "generate" ? (hydratedQrSession?.token ?? null) : null;

  useScoreQrRequesterSessionEvents(
    requesterQrToken,
    mode === "generate" && Boolean(requesterQrToken),
    {
      onRequestConsumed: () => setQrSessionConsumed(true),
      onScoresUpdated: () => {
        if (!qrScoreSync.isDirty) {
          qrScoreSync.markSynced(currentQrRequestKeyRef.current);
        }
      },
    },
  );
  ...
  useEffect(() => {
    ...
    if (!activeScoreQrRequestId || !hasPendingQrSession) return;
    if (scoreDraftRows?.matchKey === scoreDraftMatchKey) return;
    if (!isScoreFormValidForQr) return;
    qrScoreSync.markSynced(currentQrRequestKey);
  }, [
    ...
    mode,
    qrScoreSync.markSynced,
    scoreDraftMatchKey,
    scoreDraftRows?.matchKey,
  ]);
  ...
  }, [cancelActiveQrMutation, effectiveSelectedOption.kind, qrScoreSync.resetSync]);
  ...
  useEffect(() => {
    ...
    const opponentConfirmedViaPolling =
      !qrSessionConsumed &&
      wasTrackingSession &&
      !hasPendingQrSession &&
    ...
    if (opponentConfirmedViaPolling) {
  ...
  useEffect(() => {
    if (mode !== "generate") {
      opponentConsumedHandledRef.current = false;
      return;
    }
    ...
    if (opponentConsumedHandledRef.current) return;
    opponentConsumedHandledRef.current = true;
  ...
  useEffect(() => {
    ...
    attemptedAutoQrKeyRef.current = null;
    qrScoreSync.resetSync();
  }, [isScoreFormValidForQr, mode, qrScoreSync.resetSync]);
